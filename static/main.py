...
from static.RemoteHandle import *
import sys
import io
...
def Remote_Server_EngineA(FilePath,vt=True,mb=True,cust=False,ember=False):
    # print(f"Starting static analysis for: {FilePath}")
    result_dic=dict()
    Remote_Server_Engine(FilePath,vt,mb,cust,ember)
    # hashValue= hash_file(FilePath)
    # if (vt or mb):
    #     hashValue= hash_file(FilePath)
    # if vt:
